...
RunLoop和线程的关系
苹果不允许床之间创建RunLoop，它和线程是一一对应的，奇怪是保存在一个全局的字典里。线程刚创建的时候没有RunLoop,除了主线程,r而子线程如果你不获取就不会有。RunLoop销毁是发生在线程结束时。
...
git fetch origin master 删除远程分支
...
第一种方法部分类和实例都可以调用，只不过一个传(obj_getMethodClass("ClassName"),@selector(methodName))；实例调用传([obj class],@selector()methodName)
